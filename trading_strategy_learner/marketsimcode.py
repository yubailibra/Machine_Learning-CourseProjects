...
def updateOneDay(position, price, priorholding, priorcash, impact=0.005,commission=0):
    # order = 0, +/-1000, +/-2000
    # holding, price are scalar
    order = 0
    if priorholding<1000 and position>0:
        order=1000-priorholding
    if priorholding>-1000 and position<0:
        order=-1000-priorholding
    if priorholding!=0 and position==0:
        order = -priorholding
    #prices=np.array([price,1])
    sign= np.sign(order)
    absSign = np.abs(sign)

    cash = -1.0*(order * price)*(1 + sign * impact) - commission * absSign
    trade = np.array([order,cash])
    holdings = np.array([priorholding, priorcash]) + trade

    #portval = (prices * holdings).sum()

    return holdings #currentholding, currentcash
